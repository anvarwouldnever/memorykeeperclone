...
export default function ResetVerification({ route }) {
    ...
    async function verificate() {
        try {
            ...
            if(JSON.stringify(userCode) === code) {
                setError()
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.verification}>
                    <Text style={{position: 'absolute', color: 'red', fontSize: 14, top: '25%', width: ''}}>{error}</Text>
                    <View style={styles.text}>
                        ...
                        <View style={{justifyContent: 'center', width: SCREEN_WIDTH * 0.825641, alignItems: 'center', flexDirection: 'row'}}>
                            <Text style={{color: '#949494', fontSize: 14, fontWeight: '400', lineHeight: 14}}>Пароль отправлен на</Text>
                            <Text style={{color: 'white', fontSize: 14, fontWeight: '500', lineHeight: 14, marginLeft: 6}}>{email}</Text>
                        </View>
                    </View>
